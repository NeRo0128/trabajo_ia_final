...
import src.preprocessing.mappings as maps
...
def preprocessed(input_path: str, out_path: str):
    ...
    # tras separar X_train, y_train (o tras concatenar df_train)
    sm = SMOTE(random_state=42)
    X_res, y_res = sm.fit_resample(x_train, y_train)

    # y luego reconstruyes df_train_bal si lo necesitas
    df_train_bal = pd.concat([pd.DataFrame(X_res, columns=x_train.columns),
                              pd.Series(y_res, name='complications')], axis=1)
    # *Save Results (usando el SMOTE real)
    pd.DataFrame(X_res, columns=x_train.columns) \
        .to_pickle(os.path.join(out_path, "X_train.pkl"))

    pd.Series(y_res, name="complications") \
        .to_pickle(os.path.join(out_path, "y_train.pkl"))
